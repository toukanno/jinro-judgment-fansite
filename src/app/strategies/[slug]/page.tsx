...
type Article = { id: number; title: string; slug: string; summary: string; content: string; difficulty: number };

function getArticle(slug: string): Article | undefined {
  const db = getDb();
  return db
    .prepare(
      `SELECT a.id, a.title, a.slug, a.summary, a.content, a.difficulty
       FROM articles a JOIN article_categories c ON a.category_id = c.id
       WHERE a.slug = ? AND c.slug = 'strategies'`
    )
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-4 text-sm text-[var(--color-text-muted)]">
        <Link href="/strategies" className="hover:text-[var(--color-primary)]">攻略・戦略</Link>
        {" > "}
